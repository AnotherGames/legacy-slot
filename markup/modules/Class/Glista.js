...
export class Glista {
    ...
    constructor(param) {
        ...
        this.sprites = [];
        this.light = [];
        for (let atlasInd = 0; atlasInd <= 8; atlasInd++) {
            this.rnd = this.game.rnd.integerInRange(1, 5);
            let sprite = this.game.add.sprite( atlasInd * -80, 0, 'glista', `fish${this.rnd}.png`, param.parent);
            sprite.anchor.set(0.5, 0.5);
            sprite.myDeltaY = this.game.rnd.integerInRange(-30, 30);
            sprite.myScale = this.game.rnd.integerInRange(10, 15) / 10;
    ...
    */
    start(path, time = config.glista.time, callback) {
        ...
        for (let spriteInd = 0; spriteInd < 9; spriteInd++) {
