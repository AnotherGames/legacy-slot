...
export let view = (() => {
    ...
    let draw = {
        ...
        addCows: function ({
            ...
            let redIndian = game.add.sprite(0, 0, 'red_indian', null, container);

            cowboy.inputEnabled = true;
            cowboy.events.onInputDown.add(() => {
                soundController.sound.playSound({sound: 'whip', volume: 7});
            });
            redIndian.inputEnabled = true;
            redIndian.events.onInputDown.add(() => {
                soundController.sound.playSound({sound: 'whip', volume: 7});
            });
            ...
            redIndian.x = (side === 'left') ? -redIndian.width - 300 : game.width + redIndian.width + 300;
            redIndian.y = cowContainer.y;
            if (side === 'left') {
                redIndian.width = -redIndian.width;
            }
            ...
            let redIndianAnim = redIndian.animations.add('idle6', Phaser.Animation.generateFrameNames('indeets-walk_', 0, 14, '.png', 1));
            animArr.push(cow0Anim, cow1Anim, cow2Anim, cow3Anim, cow4Anim, cowboyAnim, redIndianAnim);
            ...
            game.add.tween(redIndian).to({x: delta}, time * 1000, 'Linear', true, 6000)
                .onComplete.add(() => {
        ...
        showBottleBangs: function ({callback, sprite}) {
            let game = model.el('game');
            let bangs = draw.addBangs({sprite});
        ...
        recoverBottles(number) {
            let fsBottle = model.el(`fsBottle${number}`);
            let fsMulti = model.el(`fsMulti${number}`);
            let bottleShadow = model.el(`bottleShadow${number}`);
            let brokenBottleShadow = model.el(`brokenBottleShadow${number}`);

            fsBottle.frameName = 'Bottlebang-Bang14.png';
            fsMulti.visible = true;
